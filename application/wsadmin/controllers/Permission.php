...
class Permission extends CI_Controller {
	...
	function permission($offset, $page){
		...
		$check = GetMenu();
		if(!empty($check) && !in_array($this->id_menu, unserialize($check[0]['ws_menus_temp']))){
			error_page();
		}
	...
	function edit($field_id=null, $data = null, $err = null){
		...
		if(($ses_name['ws_permission_id'] != $field_id) && ($ses_name['ws_user_acces'] == TRUE)) redirect(base_url('error'), 'refresh'); //cek session
